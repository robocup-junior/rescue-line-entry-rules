...
. All pre-mapping activities will immediately disqualify the robot for the round. Pre-mapping is the act of humans providing the robot with information about the field (e.g., location of the bstacles and the entrance to the evacuation zone, the directions for avoiding obstacles, the layout of black lines, the correct path at intersections, or the placement of victims inside the evacuation zone, etc…) before the game.


=== Start of Game

. [Intermediate]  Each team has a maximum of 6 minutes for a game. The game includes the time for calibration and the scoring run.

. [Beginner]  Each team has a maximum of 4 minutes for a game. The game includes the time for calibration and the scoring run.
...
. [Intermediate] If a lack of progress occurs in the evacuation zone, victims (including those that have rolled) shall be returned to any position within the victim area. Rescued victims are not returned. These positions are approximate and not required to be exact. Teams are not allowed to make any requests or complaints regarding the placement of the victims.
...
.. 【 Beginner】
+
The robot shall stop at the victim’s position and remain stationary while blinking a light clearly visible to the referee for a continuous duration of 5 seconds.
+
The blinking interval shall be ON 500 ms / OFF 500 ms.
+
Failure to comply with the specified blinking interval shall result in an unsuccessful detection.

.. [Intermediate] 
+
The robot shall completely remove the victim from the green circle.
The victim shall be considered detected only after it is fully outside the green circle. 
+
...
.. [Beginner] 
+
A rescue shall be considered successful when the robot detects a victim and blinks the correct color light during the identification procedure.
Each successful rescue shall be worth 10 points.
If the robot blinks an incorrect color light for the same victim at any time, the rescue points for that victim shall be canceled.
The robot shall stop at the victim and, while stationery, blink a light clearly visible to the referee for 5 seconds (see <<construction>>). 
+
The blinking interval shall be ON 500 ms / OFF 500 ms.

... Green victim: green light
...	Black victim: non-green light
...
. The game ends when: 
.. [Intermediate]   the 6 minutes of allowed game time expires 
.. [Beginner]   the 4 minutes of allowed game time expires 
