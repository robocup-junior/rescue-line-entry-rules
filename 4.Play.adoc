...
. Exit Bonus
+
A successful exit will be awarded 10 points if the robot meets the following requirements:
Exit perfoemance
..The robot must have completely entered the Evacuation Zone at least once.
..The robot must completely exit the Evacuation Zone.
..The robot must remain completely stationary for 5 seconds.
..The robot must turn on an LED for 5 seconds. (The color of the LED does not matter.)
...
.. Once the robot has completely exited the Evacuation Zone and stopped for 5 seconds.
